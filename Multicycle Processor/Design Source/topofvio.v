...
module overall_system_top (
    ...
    // --- VIO Control Wires ---
    wire rst_vio, manual_mode, manual_start;
    ...
    // ============================================================================
    // OPTIMIZATION 1: VIO Glitch Shielding (Zero VIO Switching Power)
    // ============================================================================
    ...
    // ============================================================================
    // OPTIMIZATION 2: Hard-Zero Input Gating (Eliminates Routing Power)
    // ============================================================================
    ...
    program_counter pc_inst (
        .clk(clk), .rst(rst_vio), .enable(manual_mode ? 1'b0 : pc_enable), 
    ...
    control_fsm_adaptive fsm_inst (
        .clk(clk), .rst(rst_vio), .opcode(dec_opcode), 
    ...
    register_file_adaptive regfile (
        .clk(clk), .rst(rst_vio), .read_addr_a(rs_addr), .read_addr_b(rt_addr),
    ...
    adaptive_alu_top your_alu_inst (
        .clk(clk), .rst(rst_vio), .operand_a(gated_a), .operand_b(gated_b),
    ...
    booth_mac_16bit your_mac_inst (
        .clk(clk), .rst(rst_vio), .multiplicand(gated_a), .multiplier(gated_b),
        .accumulator(mac_lo),
    ...
    data_memory dmem (
        .clk(clk), .write_enable(mem_write_en), 
